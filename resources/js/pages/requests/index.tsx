...
function TimelineStep({
    ...
    return (
        <div className="flex items-center gap-2">
            <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 text-xs font-bold ${active ? 'border-teal-600 bg-teal-600 text-white' : 'border-gray-200 bg-white text-gray-300'}`}>
                {active ? '✓' : ''}
            </div>
            <div className="min-w-0">
                <div className={`text-xs font-medium ${active ? 'text-gray-900' : 'text-gray-400'}`}>{label}</div>
                {date && <div className="text-xs text-gray-400">{new Date(date).toLocaleDateString()}</div>}
            </div>
            {!last && <div className={`mx-1 h-px w-8 ${active ? 'bg-teal-200' : 'bg-gray-100'}`} />}
        </div>
    );
}
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">Track the status of your review requests</p>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium text-gray-500">Opened</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-bold text-blue-600">{stats.opened}</div>
                            {stats.sent > 0 && (
                                <p className="mt-1 text-sm text-gray-500">{Math.round((stats.opened / stats.sent) * 100)}% open rate</p>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            ...
                            {stats.sent > 0 && (
                                <p className="mt-1 text-sm text-gray-500">{Math.round((stats.reviewed / stats.sent) * 100)}% conversion</p>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium text-gray-500">No Response</CardTitle>
                        </CardHeader>
                        <CardContent>
                    ...
                    <CardContent className="p-0">
                        {requests.data.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-16 text-center">
                                ...
                                <h3 className="mb-1 text-base font-semibold text-gray-900">No requests sent yet</h3>
                                <p className="mb-4 text-sm text-gray-500">Use Quick Send to send your first review request.</p>
                            </div>
                        ) : (
